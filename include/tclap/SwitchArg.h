...
namespace TCLAP {
	...
	inline bool SwitchArg<T_Char, T_CharTraits, T_Alloc>::lastCombined(StringType& combinedSwitches) {
		for (const CharType& ch_combinedSwitches : combinedSwitches) if (ch_combinedSwitches != Arg<T_Char, T_CharTraits, T_Alloc>::blankChar()) return false;
		return true;
	}
	...
	inline bool SwitchArg<T_Char, T_CharTraits, T_Alloc>::combinedSwitchesMatch(StringType& combinedSwitches) {
		...
		for (CharType& ch_combinedSwitches : combinedSwitches)
			if (!_flag.empty() && ch_combinedSwitches == _flag.front() && _flag.front() != Arg<T_Char, T_CharTraits, T_Alloc>::flagStartString().front()) {
				// update the combined switches so this one is no longer present
				// this is necessary so that no unlabeled args are matched
				// later in the processing.
				//combinedSwitches.erase(i,1);
				ch_combinedSwitches = Arg<T_Char, T_CharTraits, T_Alloc>::blankChar();
				return true;
			}
	...
	template<typename T_Char, typename T_CharTraits, typename T_Alloc>
	inline bool SwitchArg<T_Char, T_CharTraits, T_Alloc>::processArg(std::size_t& idx_arg, StringVectorType& args) {
		...
		// if a substring matches the flag as part of a combination
			...
			if (combinedSwitchesMatch(args[idx_arg]))
				throw(CmdLineParseException<T_Char, T_CharTraits, T_Alloc>(StringConvertType::fromConstBasicCharString("Argument already set!"), toString()));
			...
			return lastCombined(args[idx_arg]);
		} else
			return false;
	}
